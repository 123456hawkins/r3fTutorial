...
interface PROPS{
    currentStepSpring:SpringValue<number>
}
const NB_LIGHTS = 42;
export const CarouselLights = ({ currentStepSpring  }:PROPS) => {
      ...
      {[...Array(NB_LIGHTS)].map((_item, index) => {
        ...
        return (
          <mesh key={index} scale={[0.2, 0.2, 0.2]} position={[x, -1, z]}>
            <sphereGeometry />
            <meshStandardMaterial toneMapped={false} emissive={emissiveColor} emissiveIntensity={isActive ? 2 : 0} />
          </mesh>
        );
      })}
    </>
