...
export function FerrisWheel(props) {
  const { nodes, materials } = useGLTF("/models/springAnimationModel/ferris-wheel/model.gltf");
  const lightRef=useRef(null!)
  useFrame((state)=>{
    const speed=state.clock.elapsedTime*6
    const distance=Math.sin(state.clock.elapsedTime)*0.8
    lightRef.current.position.x=Math.sin(speed)*distance
    lightRef.current.position.y=Math.cos(speed)*distance
  ...
  return (
    <group {...props} dispose={null}>
      <group position={[0,1.25,0]}>
        <mesh ref={lightRef} scale={[0.04,0.04,0.04]}>
        <pointLight intensity={1} color={'yellow'}/>
        <sphereGeometry/>
        <meshStandardMaterial {...hawkinsMaterials.yellow} emissive={hawkinsMaterials.yellow.color} emissiveIntensity={4} toneMapped={false}/>
...
useGLTF.preload("/models/springAnimationModel/ferris-wheel/model.gltf");
